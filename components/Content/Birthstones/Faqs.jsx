...
import { handleCreateBirthStones, handleCreateGuide, handleUpdateBirthStones, handleUpdateGuide } from "@/API/api";

const Faqs = ({ handleBirthStones, type, title, sectionData, fetchData }) => {
  ...
  useEffect(() => {
    if (sectionData) {
      setFormData({
        ...
        questions: sectionData.faqs?.questions || [{ question: "", answer: "" }],
      });
      setQuestions(sectionData.faqs?.questions || [{ question: "", answer: "" }]);
    }
  }, [sectionData]);
  ...
  const handleSubmit = async (e) => {
    ...
    const bodyData = {
      title: title,
      faqs: { title: formData.title, questions: updatedQuestions },
    };
    ...
    try {
      ...
      if (type === "create") {
        response = await handleCreateBirthStones(bodyData, false);
      } else if (type === "edit") {
        response = await handleUpdateBirthStones(bodyData, sectionData._id, false);
      }
