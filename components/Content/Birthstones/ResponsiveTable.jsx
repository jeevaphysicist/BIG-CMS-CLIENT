...
import { handleDeleteBirthStones, handleDeleteGuide, handleDeleteSitepage, handleUpdateBirthStonesStatus, handleUpdateGuideStatus, handleUpdateSitepageStatus } from "@/API/api";
import AlertModel from "@/components/AlertModal";

const DraggableRow = ({ UpdateStatus, fetchData, row, handleBirthStones, handleType, handleSetEditData }) => {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const response = await handleDeleteBirthStones(collectID);
    ...
    >
      ...
      <td className="px-4 py-4 text-[14px]">
        <div className="flex items-center gap-5">
          ...
          <button className="text-[20px] text-[#475467]" onClick={() => { handleBirthStones(); handleType('edit'); handleSetEditData(row); }}>
            <FiEdit2 />
          </button>
        </div>
      </td>
...
const ResponsiveTable = ({ searchQuery, initialData, handleSetEditData, fetchData, handleType, handleBirthStones }) => {
  ...
  const UpdateStatus = async (status, id) => {
    try {
      const response = await handleUpdateBirthStonesStatus({ status }, id);
      if (response.status >= 200 && response.status <= 209) {
        fetchData();
        toast.success(response.data.message);
      } else {
