...
import {  handleUpdateGemstoneCertificationStatus } from "@/API/api";

const DraggableRow = ({ UpdateStatus, row, handleGemstoneCertification, handleSetEditData }) => {
    ...
    >
      ...
      <td className="px-4 py-4 text-[14px]">
        <div className="flex items-center gap-5">
          ...
          <button className="text-[20px] text-[#475467]" onClick={() => { handleGemstoneCertification();  handleSetEditData(row); }}>
            <FiEdit2 />
          </button>
        </div>
      </td>
...
const ResponsiveTable = ({ searchQuery, initialData, handleSetEditData, fetchData,  handleGemstoneCertification }) => {
  ...
  const UpdateStatus = async (status, id) => {
    try {
      const response = await handleUpdateGemstoneCertificationStatus({ status }, id);
            ...
            <tr>
              <th className="px-4 text-[12px] text-start py-3 border-b-1 text-[#475467] text-nowrap">
                <span className="flex items-center gap-2">
                  Certification Type <PiArrowDownBold />
                </span>
              </th>             
