...
import { handleDeleteSitepage, handleUpdateRingSizeGuideStatus, handleUpdateSitepageStatus } from "@/API/api";
import AlertModel from "@/components/AlertModal";

const DraggableRow = ({ UpdateStatus, row, handleSizeGuide, handleSetEditData }) => {
    ...
    >
      ...
      <td className="px-4 py-4 text-[14px]">
        <div className="flex items-center gap-5">
          ...
          <button className="text-[20px] text-[#475467]" onClick={() => { handleSizeGuide();  handleSetEditData(row); }}>
            <FiEdit2 />
          </button>
        </div>
      </td>
...
const ResponsiveTable = ({ searchQuery, initialData, handleSetEditData, fetchData,  handleSizeGuide }) => {
  ...
  const UpdateStatus = async (status, id) => {
    try {
      const response = await handleUpdateRingSizeGuideStatus({ status }, id);
