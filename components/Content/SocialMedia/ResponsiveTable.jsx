...
import {  handleDeleteSocialmedia,  handleUpdateSocialmediaStatus } from "@/API/api";
import AlertModel from "@/components/AlertModal";

const DraggableRow = ({ UpdateStatus, fetchData, row, handleSocialMedias, handleType, handleSetEditData }) => {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const response = await handleDeleteSocialmedia(collectID);
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-4 font-regular text-[14px]">
        <p className="text-balance w-[200px]">{row.link ? row.link : "-"}</p>
      </td>
      ...
      <td className="px-4 py-4 text-[14px]">
        <div className="flex items-center gap-5">
          ...
          <button className="text-[20px] text-[#475467]" onClick={() => { handleSocialMedias(); handleType('edit'); handleSetEditData(row); }}>
            <FiEdit2 />
          </button>
        </div>
      </td>
...
const ResponsiveTable = ({ searchQuery, initialData, handleSetEditData, fetchData, handleType, handleSocialMedias }) => {
  ...
  const UpdateStatus = async (status, id) => {
    try {
      const response = await handleUpdateSocialmediaStatus({ status }, id);
      if (response.status >= 200 && response.status <= 209) {
        fetchData();
        toast.success(response.data.message);
      } else {
